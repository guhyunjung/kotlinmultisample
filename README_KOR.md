...
[https://klibs.io/kotlin-multiplatform](https://klibs.io/kotlin-multiplatform)에서 Kotlin Multiplatform 라이브러리를 찾아보세요.

## 프로젝트 구조 및 패키지 설명

이 프로젝트는 다음과 같은 주요 모듈로 구성되어 있습니다.

### 1. `composeApp` (Compose Multiplatform UI)
- **역할**: 안드로이드, 데스크탑(JVM), iOS(예정) 등 다양한 플랫폼에서 실행되는 UI 애플리케이션입니다.
- **주요 패키지**: `com.example.kotlinmultisample`
- **구조**:
  - `commonMain`: 모든 플랫폼에서 공유되는 UI 코드 (`App.kt`, `CustomNavigation.kt`)
  - `androidMain`: 안드로이드 전용 설정 및 코드
  - `jvmMain`: 데스크탑 전용 코드

### 2. `server` (Backend Server)
- **역할**: Spring Boot 기반의 백엔드 서버입니다. 데이터베이스 연동 및 API를 제공합니다.
- **주요 패키지**: `com.example.kotlinmultisample`
- **상세 구조**:
  - `Application.kt`: Spring Boot 애플리케이션 진입점
  - `server/`: 백엔드 로직이 포함된 패키지
    - `controller/`: 웹 요청을 처리하는 컨트롤러
    - `service/`: 비즈니스 로직을 담당하는 서비스 계층
    - `repository/`: 데이터베이스 접근을 담당하는 리포지토리 (JPA 등)
    - `entity/`: 데이터베이스 테이블과 매핑되는 엔티티 클래스

### 3. `shared` (Shared Logic Library)
- **역할**: 클라이언트(ComposeApp, WebApp)와 서버 간 공유하거나, 플랫폼 독립적인 비즈니스 로직을 담고 있는 라이브러리입니다.
- **주요 패키지**: `com.example.kotlinmultisample`
- **구조**:
  - `commonMain`: `Calculator`, `Greeting`, `Platform` 등의 공통 로직 클래스 포함

### 4. `webApp` (React Frontend)
- **역할**: React 기반의 웹 프론트엔드 애플리케이션입니다.
- **특징**: `shared` 모듈에서 컴파일된 Kotlin/JS 라이브러리를 사용하여 로직을 공유할 수 있습니다.
- **구조**:
  - `src/index.tsx`: 진입점
  - `src/components/`: 리액트 컴포넌트
